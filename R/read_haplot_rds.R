...
#' folder of data files can be read and joined with this function (note that this
#' will ignore the '_posinfo' and '_annotate' files generated by microhaplot be default).
#' If you only want a couple of runs read in rather than your whole microhaplot bin, or
#' just one run, you can feed this function a list of .rds files (just make sure they're
#' not '_posinfo' or '_annotate' files)
#' @param datapath path to microhaplot rds files. If this is a directory, then all the files within it
#' ending with `.rds` (but not '_posinfo.rds', or '_annotate.rds') will be processed. If `datapath`
#' is single file with an rds extension then only that file will be loaded. If it is  a vector of file paths,
#' at least one of which has a `.rds` extension, then the function will attempt to load all of the files.
#' @export
