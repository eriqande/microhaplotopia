...
#### Making your datset old-microhaplotopia compatible

Need to make your data compatible with an old Neil dataset or another dataset that
was processed with old microhaplotopia?  Here's how Neil used to filter data!

```{r}
#set your filtering crieria like above for light_filters
haplotype_depth <- 5
total_depth <- 20
allele_balance <- 0.3

#Apply Neil's filtering regime

hap_NX_filter_Neil <- hap_raw %>%
...
At this point Neil would then filter out genotypes with extra alleles (which we
do next!) and then add extra rows for homozygotes (which I do right before converting
these genotypes to other formats because there's really no reason to carry around
those extra rows until then).
...
If you think these are just one-off issues, you can remove just the genotypes with
the extra alleles with a simple anti_join like below:

```{r}
hap_fullFilter <- hap_NX_filter %>%
  anti_join(BadFishGenos)
```
...
```{r}
missing_samples <- find_missing_samples(hap_raw, hap_fullFilter)
...
```{r}
hap_miss_example <- hap_fullFilter %>%
...
reruns)" section above).  Since we didn't have any duplicates in this example, 
we're just going to use our filtered dataset.

```{r}
hap_NoLowTypers <- filter_missing_data(hap_fullFilter, n_locs = 100)
```
