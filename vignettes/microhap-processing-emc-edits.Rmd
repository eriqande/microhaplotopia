...
You may have intentional reruns (i.e. samples with the same sample ID, but were 
run on different miseq runs for some reason or another).  If this is the case, we
have a bit of code that can identify samples with the same indiv.ID but different
groups.
...
And now we can run find_duplicates() to find duplicate samples in our fake
dataset
```{r}
...
If we know we were rerunning these samples because it failed another run, we can 
just keep whichever run has more loci passing filter with the "drop" option.  In
our fake case, we know that the original data (group "WRcarc") has the most loci
passing filter, so if we run this, we should only have data from the "WRcarc" 
group for fish "chinook18699"
...
hap_HomsFixed <- add_homozygote_second_allele(hap_NoLowTypers)
...
```
And we can see we don't have any matching samples in our dataset.
