...
```{r}
hap_raw <- read_unfiltered_observed("../inst/extdata/")

hap_raw_file <- read_unfiltered_observed("../inst/extdata/gtseq65_observed_unfiltered_haplotype.csv.gz")
```
...
```{r}
head(hap_raw) %>%
  kable("html") %>% kable_styling(full_width = FALSE)
```
...
```{r, include = FALSE}
tibble(indiv.ID = rep("fish1",2), locus = rep("locus_A",2), haplo = c("AA", "AT"), depth = c(30,20)) %>%
  kable("html") %>% kable_styling(full_width = FALSE)
```
...
```{r}
hap_fil1 <- filter_raw_microhap_data(hap_raw, haplotype_depth = 5,
...
calculate_missing_data(hap_miss_example) %>% head() %>%
  kable("html") %>% kable_styling(full_width = FALSE)
```
...
```{r}
mhap_transform(hap_fil3, program = '2columnformat_haplotype') %>% head() %>%
  kable("html") %>% kable_styling(full_width = FALSE)
```
...
two_col_numeric$data %>% head() %>%
  kable("html") %>% kable_styling(full_width = FALSE)

two_col_numeric$key %>% head() %>%
  kable("html") %>% kable_styling(full_width = FALSE)
```

```{r}
mhap_transform(hap_fil3, "CKMRsim") %>% head() %>%
  kable("html") %>% kable_styling(full_width = FALSE)
```

```{r}
mhap_transform(hap_fil3, "rubias") %>% head() %>%
  kable("html") %>% kable_styling(full_width = FALSE)
```
